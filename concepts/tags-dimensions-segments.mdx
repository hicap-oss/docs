...
description: 'How the Hicap AI Gateway labels and groups traffic so you can slice usage and spend by what you send on each request'
icon: 'tags'
---
...
Some dimensions are best set once per service so spend can be sliced consistently across every request it makes. Dimensions like `env`, `system`, and `channel` fall into this group — set them once in your client and they ride along on everything. Per-call dimensions like `feature` or `agent` then narrow the activity within a service.
