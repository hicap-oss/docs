...
A **Connection** sits within an Application and is what Keys are issued against. It is the level at which you group Keys that belong together, and it gives you:

- **A Key pair** — a primary and a secondary Key, issued under the Connection.
- **A reporting boundary** — a scope you can slice usage and spend by on its own, beneath the Application.

An Application can have more than one Connection, so you can keep distinct parts of the same product separately credentialed and separately reportable — for example an experimentation Connection alongside a production one.
...
- **Changes stay contained.** Regenerating a Key does not disturb the Connection, and removing a Connection does not disturb the Application. You can rotate credentials or add and remove Connections without reorganizing anything above.
...
<AccordionGroup>
  ...
  <Accordion title="When to add another Connection">
    Add a Connection when part of an Application should be credentialed and reported on separately — for example experimentation alongside production, or one Connection per environment. Each Connection brings its own pair of Keys.
  </Accordion>
