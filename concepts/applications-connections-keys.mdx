...
An **Application** is the thing you are building or buying that consumes AI — a product, service, or workload. Applications are the rollup point for Keys: every Key ultimately reports its usage into the Application it belongs to, which makes the Application the natural unit to plan, budget, and report on.

Group your work into Applications the way you think about it internally, so reporting matches how you already plan.

### Connection

A **Connection** binds an Application to a model provider — OpenAI, Anthropic, Google, ElevenLabs, and others. It carries the provider choice and its configuration, and it is what Keys are issued against.

An Application can have more than one Connection, so a single product can draw on several providers at once.
...
- **Changes stay contained.** Regenerating a Key does not disturb the Connection, and removing a Connection does not disturb the Application. You can rotate credentials or add a provider without reorganizing anything above.
...
<AccordionGroup>
  ...
  <Accordion title="When to add another Connection">
    Add a Connection when an Application needs a different provider, or a different configuration of the same provider — for example one provider for chat and another for voice. Each Connection brings its own pair of Keys.
  </Accordion>
